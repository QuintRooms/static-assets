...
export default class Algolia {
    constructor(site_config, page_name, utilities) {
        this.site_config = site_config;
        this.page_name = page_name;
        this.utilities = utilities;
        // this.member_token = this.utilities.getMetaTagContent('memberToken');
        this.lat_lng = undefined;
        this.this.default_lat_lng = undefined;
    ...
    init() {
        ...
        jQuery('#theBody').on('arnMapLoadedEvent', () => {
            if (this.search_params.has('locationlabel') || this.search_params.has('points')) return;
            this.hideArnSearchElements('img.arn-green-marker-icon');
        });
     ...
     */
    remove_arn_search_bar(selector) {
        if (!document.querySelector(selector)) return;
     ...
     */
    insert_algolia_search(page, selector, adjacent_location, html) {
    ...
    prepopulate_inputs_on_search_hotels() {
        ...
        let destination;
        if (this.search_params.get('destination') !== null) {
            destination = this.search_params.get('destination');
        } else {
            destination = this.original_params_url.get('destination');
        }
     ...
     */
    setDropdownIndex(dropdown_selector) {
    ...
    remove_city_search_for_event() {
        ...
        if (this.site_config.site_type.toLowerCase() === 'cug' || this.site_config.site_type.toLowerCase() === 'retail') return;
        this.utilities.waitForSelectorInDOM('.algolia-places').then(() => {
     ...
     */
    getDestinationForCUG(inputSelector) {
     ...
     */
    getEventOrginalParams(paramString) {
        const param = this.original_params_url.get(paramString);
        return param;
    }
    ...
    submitListener() {
        document.querySelector('form#searchForm').addEventListener('submit', (e) => {
            ...
            const url = `${origin}/v6/?type=geo&siteid=${this.site_config.site_id}&pagesize=10&${this.site_config.distance_unit}`;
            ...
            if (this.lat_lng) {
                lat = this.lat_lng.lat;
                lng = this.lat_lng.lng;
            } else if (this.default_lat_lng) {
                lat = this.default_lat_lng.lat;
                lng = this.default_lat_lng.lng;
            } else if (!this.lat_lng && !this.default_lat_lng && this.page_name === 'search-results') {
                lat = this.original_params_url.get('latitude');
                lng = this.original_params_url.get('longitude');
            }
            ...
            appendParamsToURL({
                ...
                rooms: {
                    key: 'rooms',
                    value: this.setDropdownIndex('select#rooms'),
                },
                adults: {
                    key: 'adults',
                    value: this.setDropdownIndex('select#adults'),
                },
                currency: {
                    key: 'currency',
                    value: this.site_config.currency,
                },
                amenities: {
                    key: 'amenities',
                    value: this.applyFilters('#AmentitiesContainer .ArnSearchField div', 'lblAmenities'),
                },
                stars: {
                    key: 'propertyclasses',
                    value: this.applyFilters('#PropertyClassesContainer .ArnSearchField div', 'lblRating'),
                },
                propertyType: {
                    key: 'propertytypes',
                    value: this.applyFilters('#PropertyTypesContainer .ArnSearchField div', 'lblPropertyType'),
                },
                destination: {
                    key: 'destination',
                    value: this.getDestinationForCUG('input#address-input'),
                },
                optionalHotel: {
                    key: 'hotelname',
                    value: this.get_optional_hotel_name(),
                },
            });
            ...
            if (this.page_name === 'search-results') {
                appendParamsToURL({
                    properties: {
                        key: 'properties',
                        value: this.getEventOrginalParams('properties'),
                    },
                    utm_sorce: {
                        key: 'utm_sorce',
                        value: this.getEventOrginalParams('utm_sorce'),
                    },
                    locationLabel: {
                        key: 'locationlabel',
                        value: this.getEventOrginalParams('locationlabel'),
                    },
                    radius: {
                        key: 'radius',
                        value: this.getEventOrginalParams('radius'),
                    },
                    groupId: {
                        key: 'groupid',
                        value: this.getEventOrginalParams('groupid'),
                    },
                    cid: {
                        key: 'cid',
                        value: this.getEventOrginalParams('cid'),
                    },
                });
            }
    ...
    places_autocomplete() {
        (() => {
            ...
            places_autocomplete.on('change', function resultSelected(e) {
                ...
                this.lat_lng = e.suggestion.latlng;
            });
