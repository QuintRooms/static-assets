...
import SiteConfig from './94605-config';
...
class ChildPortal extends BasePortal {
    ...
    async addWeekendButtons() {
        if (document.querySelector('meta[name="siteId"]').content !== '94605') return;

        await utilities.waitForSelectorInDOM('header');
        document.querySelector('header').insertAdjacentHTML(
            'beforeend',
            `<div class="event-week pull-right">
            <a id="week-one" target="_blank" href="https://aclfestival.quintrooms.com/group-event?id=72019">Book Weekend One</a>
            <a id="week-two" target="_blank" href="https://aclfestival.quintrooms.com/group-event?id=72023">Book Weekend Two</a>
        </div>`
        );
    }
    ...
    async addUtmTrackingToUrls(url) {
        ...
        if (check_in === '10/4/2024') weekend_btns[0].style.display = 'none';
        if (check_in === '10/11/2024') weekend_btns[1].style.display = 'none';
