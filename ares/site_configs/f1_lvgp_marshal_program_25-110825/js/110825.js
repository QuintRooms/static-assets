...
import SiteConfig from './110825-config';
...
async function updatePropDetailPricing() {
    ...
    if (document.querySelector('.SinglePropDetail')) {
        ...
        roomRateRowArr.forEach((room) => {
            ...
            const roomDescriptionUpdatedText = roomDescriptionText
                .substring(roomDescriptionText.indexOf('y') + 1)
...
async function updateCheckoutDescription() {
    await utilities.waitForSelectorInDOM('#theBody');
    if (document.querySelector('.CheckOutForm')) {
        ...
        const roomDescriptionUpdatedText = roomDescriptionText
            ...
            .replace('undefined', '');
        console.log('roomDescriptionUpdatedText = ' + roomDescriptionUpdatedText);
        roomDescriptionElement.innerHTML = `<span style="font-weight: 700; display: block;">${roomTitleText} <span style="font-weight: 300; display: block;">${roomDescriptionUpdatedText}</span></span>`;
